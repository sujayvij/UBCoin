...
public class MultiThreadUBCoinClient implements Runnable {
	static Wallet w21 = new Wallet();
	static Wallet w22 = new Wallet();
	// The client socket
	...
	public static void main(String[] args) {
		...
		if (clientSocket != null && os != null && is != null) {
			try {
				...
				new Thread(new MultiThreadUBCoinClient()).start();
				DataOutputStream dos = new DataOutputStream(os);
				while (!closed) {
					String send = inputLine.readLine().trim();

					if (send.contains("file")) {
						// String text = readWrite.readFile("F:/ubtx/tx14.txt");
						// os.println("#tx"+text);
					} else
						os.println(send);
